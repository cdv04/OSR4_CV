...
data.list<-lapply(data.list, function(x){
      x["nbED"]<-dim(x)[1]  
      x["nbSpecies"]<-nlevels(droplevels(x["SpeciesComp"])
     return(x)
})
      
      
      lapply(DescriCrue.list, function(x){
      sora %>%
          filter(Date > x["Date_debut"])%>%
          filter(Date < x["Date_fin"])%>%
