...
```{r data}
sora <- read.csv2("data/SORA_GAMMA_MES_DISSOUT_01092017.csv")
...
sora$Date <- date(sora$Date) #☻ necessaire pour afficher les mois sur l'axe des x des grpahs
...
ggplot(sora, aes(x=Date,y=Discharge))+
...
```{r charge}
ggplot(sora, aes(x=Date,y=Charge))+
  geom_point(aes(colour=RegimeHydro), size=1) +
...
sora$ReturnPeriod <- sora$Discharge %>%
